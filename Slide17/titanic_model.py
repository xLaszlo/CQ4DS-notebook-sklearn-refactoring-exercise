...
class TitanicModelCreator:
    ...
    def run(self):
        ...
        passengersMap = {p.pid: p for p in passengers}
        X_train = pd.DataFrame([passengersMap[pid].dict() for pid in train_pids])
        y_train = [passengersMap[pid].is_survived for pid in train_pids]
        X_test = pd.DataFrame([passengersMap[pid].dict() for pid in test_pids])
        y_test = [passengersMap[pid].is_survived for pid in test_pids]
